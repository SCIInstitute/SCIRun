...
It is import for the name of each port to be unique, including all the inputs and outputs.  
...
\begin{figure}[H]
\centerline{
\scalebox{0.35}{\includegraphics{SCIRun5ModuleGeneration_Figures/QT-editor.png}}}
...
You can change the size and placement of objects with the property manager, but make sure that you leave some buffer space as some OS will interpret the file slightly differently.  
...
The module dialog cc file is used with Qt to establish the functionality of module UI.  
...
The module dialog code is mostly for passing data between the module and the UI and changing the UI when require by the module.  
Parameters and inputs can be passed straight to the algorithm, as templated in this example.  
...
However, the module algorithms have more in common that it first appears.
...
#endif 
\end{verbatim}
Key difference that may occur in the algorithm header file are the function and variable declarations.
...
This template shows the algorithm using some of the defined names from \emph{Core/Algorithms/Base/AlgorithmVariableNames.h}, using the Variable namespace.
...
If desired, the final version of the header file is in the source code: \emph{src/Modules/Examples/TestModuleSimple.h}.
...
For comparison, the final version of the module code file is in the source code: \emph{src/Modules/Examples/TestModuleSimple.cc}.
...
First, delete the check box and input scroll wheel widgets and delete the \verb|knob 2| label.
Next, add a \verb|line edit widget| by finding it in the \emph{Widgdet Box} window (in the \emph{Input Widget} section) and clicking and dragging it next to the remaining label.  
...
If you load the module, you should she the default message (\verb|"[Insert message here]"|) in the input field.  
...
The module UI code files are in \emph{src/interface/Modules/Examples/}, \emph{TestModuleSimpleUIDialog.cc}, \emph{TestModuleSimpleUIDialog.h}, and \emph{TestModuleSimpleUIDialog.ui}.  
...
As mention in the chapter introduction, we are going to create a module called SortMatrix that will sort the entries of a matrix in ascending or descending order.  
This module will use a simple simple quick sort algorithm with a \href{https://en.wikipedia.org/wiki/Quicksort#Lomuto_partition_scheme}{Lomuto partition scheme}.  
...
#endif
\end{verbatim}
The import differences in this example is that the namespace and type of ports are different, and to leave the \emph{setStateDefaults()} function without brackets so it can be set in the cc file.  
...
The final module header and cc files are included in the source code in \emph{src/Modules/Examples/}.  \emph{SortMatrix.cc} and \emph{SortMatrix.h}.
...
It may be easier to copy a previously created UI file and modify it that to create one from scratch.  
...
\begin{figure}[H]
\scalebox{0.5}{\includegraphics{SCIRun5ModuleGeneration_Figures/algo_ui.png}}
...
The final step in creating this SortMatrix module is to create the module algorithm code.  
...
The final module algorithm header and cc files are included in the source code in \emph{src/Core/Algorithms/Examples/}.  \emph{SortMatrixAlgo.cc} and \emph{SortMatrixAlgo.h}.
...
This would entail get SCIRun to build with just the bare minimum of the algorithm code (only \verb|run_generic| with no calls other functions) then to add the other functions in a step by step manner.  
This allows for easier debugging and a more systematic process to get the module working.  
Check Section~\ref{sec:errors} for common build problem.
...
If this or another module is not behaving as expected, change the output of some functions and the modules to be some of the intermediate steps, or use \verb|std::cout<< "message" <<std::endl;| to print values as the code is run.  
Unit Test can also find some bugs in module code.  
...
I may be easiest to copy an existing file and change it.
...
Virtually all modules in SCIRun 4 did not have header files for the module code, so one will need to be created for it.  
...
Make sure that the namespaces used are correct (at least two,Fields/Math/etc and Networks, are needed, Section~\ref{sec:modulecc}).
...
Add \verb|staticInfo_| varible as in Section~\ref{sec:modulecc}.
...
There will probably be a warning about creating UI because the code is expecting one and there isn't one.  
Make sure all the ports are there and are correctly names (hover the mouse over the port).  
...
Copy the algorithm code from SCIRUn 4 to the appropriate directory within \emph{src/Core/Algorithms/} (some algorithms have been copied and not ported to SCIRun 5).
If there was no algorithm code in SCIRun 4, copy a similar module algorithm code or the template code found in \emph{src/Core/Algorithms/Template} and modify the names to match the module name with `Algo' added to it.  
...
Include the algorithm header in the module code.  
Build SCIRun. If there are no build errors, start adding the commented out code as described in the next section.
Commit all changes to the local branch.
...
This step of converting the modules is the most specific to the module and could require the most experience to know the various functions that may have been changed in SCIRun 5. 
However, there may be some modules that require very little code changes in this step.  
Section~\ref{sec:changes} provides some examples of commonly used functions in SCIRun 4 and the replacement in SCIRun 5.
