...
Also of note, the default values for the UI are set in the '@AlgorithmName@Algo()' function, and then in the module code, the 'setStateDefault ' function pulls the values from the algorithm.  
...
\emph{Sort} is the main helper function, which further processes the data into a format that be quickly sorted.  
...
This would entail get SCIRun to build with just the bare minimum of the algorithm code (only \emph{run\_generic} with no calls other functions) then to add the other functions in a step by step manner.  
