...
#include "Core/ctkValueProxy.h"
#include "Core/ctkPimpl.h"
...
void ctkDoubleSpinBoxPrivate::onValueProxyModified()
{
  ...
  if (oldDecimals != q->decimals())
    {
    Q_EMIT q->decimalsChanged(q->decimals());
    }
}
...
void ctkDoubleSpinBox::setValueAlways(double newValue)
{
  ...
  if (valueModified && !signalsEmitted)
    {
    Q_EMIT valueChanged(d->InputValue);
    Q_EMIT valueChanged(QString::number(d->InputValue, 'f', d->SpinBox->decimals()));
    }
}
