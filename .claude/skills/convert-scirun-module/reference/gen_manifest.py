...
import re, os, glob, csv, subprocess, sys

REPO = subprocess.check_output(['git','rev-parse','--show-toplevel'],text=True).strip()
os.chdir(REPO)
OUT = os.path.join('.claude/skills/convert-scirun-module/reference/backlog_manifest.csv')

def loc(path):
    try:
        with open(path, errors='ignore') as f: return sum(1 for _ in f)
