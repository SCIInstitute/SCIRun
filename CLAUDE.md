...
- `WITH_<DEP>` — an optional third-party dependency. The Superbuild fetches and builds it; the inner build compiles the code that uses it. `WITH_PYTHON`, `WITH_TETGEN`, `WITH_OSPRAY`, `WITH_VTK`.
...
- `BUILD_WITH_` is retired. Do not add new flags with it. `BUILD_HEADLESS` predates the rule and is being renamed (#2671); `BUILD_WITH_PYTHON` and `BUILD_OSPRAY` still work for one release with a deprecation warning.
